...
import { PlanPedagogiqueV2 } from '@/components/plan-pedagogique-v2';
...
export default function StageDetailPage() {
    ...
    // --- State for UI ---
    const initialTab = (searchParams.get('tab') as 'séances' | 'programme' | 'suivi-bis') || 'séances';
    const [activeTab, setActiveTab] = useState<'séances' | 'programme' | 'suivi-bis'>(initialTab);
        ...
        <div className="space-y-6">
            ...
            <Card className="overflow-hidden relative bg-stage-header bg-cover bg-center">
                ...
                <CardContent className="relative text-white p-6 space-y-6">
                    ...
                    <Tabs
                        ...
                        onValueChange={(value) =>
                            setActiveTab(
                                value as 'séances' | 'programme' | 'suivi-bis'
                            )
                        }
                        className="w-full"
                    >
            ...
            <div className="mt-6">
                {activeTab === 'séances' && stageId && (
                    <SessionsManager
                        stageId={stageId}
                        // On passe les objectifs du programme sélectionnés
                        // pour alimenter le sélecteur d'objectifs par étape
                ...
                {activeTab === 'programme' && stageId && (
                    <ProgrammeBuilder
                        ...
                        onSave={(newlySelectedCards) => {
                            // newSelectedCards = fiches objectifs choisies dans Programme
                ...
                {activeTab === 'suivi-bis' && stageId && (
                    // Suivi BIS dans l'onglet: rendre le composant directement
                    // plutôt qu'avec une iframe qui recharge la layout (sidebar).
